...
const PasswordResetRequestScreen = () => {
  ...
  const handleResetRequest = async () => {
    ...
    try {
      const response = await fetch(`${baseUrl}api/auth/reset-password`, { // Adjust URL
        method: 'POST',
  ...
  return (
    <View style={[styles.container, theme === 'dark' ? styles.darkContainer : styles.lightContainer]}>
      ...
      {message && <Text style={[styles.message, theme === 'dark' ? styles.darkMessage : styles.lightMessage]}>{message}</Text>}
    </View>
  );
};
