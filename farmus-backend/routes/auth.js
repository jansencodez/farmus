...
const mpesaSecurityCredential = process.env.MPESA_SECURITY_CREDENTIAL;
const mpesaB2CQueueUrl = process.env.MPESA_B2C_QUEUE_URL;
const mpesaB2CResultUrl = process.env.MPESA_B2C_RESULT_URL;
const mpesaB2CTimeoutUrl = process.env.MPESA_B2C_TIMEOUT_URL;
...
router.post('/signin', async (req, res) => {
  ...
  try {
    ...
    // Log retrieved user information for debugging (remove in production)
    console.log('User found:', user);

    // Log the plaintext and hashed password for comparison (remove in production)
...
router.post('/mpesa/b2c/result', async (req, res) => {
  ...
  // Handle the result from M-Pesa and update your records as necessary
  res.status(200).send('Result received');
});

// Timeout URL
router.post('/mpesa/b2c/timeout', async (req, res) => {
  console.log('M-Pesa B2C Timeout:', req.body);
  // Handle the timeout from M-Pesa
